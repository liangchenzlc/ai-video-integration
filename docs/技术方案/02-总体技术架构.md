...
| 原调用恢复 | POST /projects/{id}/calls/{callId}/recovery，action=query/download/stop_waiting/cancel_remote | T04、T06 |
...
表中为接口组摘要，具体方法/字段/分页以 [业务 OpenAPI](./契约/business.openapi.json) 与模块设计为准。原任务 recovery 使用 POST 表达恢复操作，其内部不得提交新的生成。

契约约定：

- 请求使用 requestId 关联诊断；项目操作携带 projectSessionId，写操作携带 expectedRevision。
- 响应统一 `{requestId,data}`；写操作 data 为含 committedRevision 的 MutationReceipt，读取 data 为对应 DTO。错误统一 `{requestId,error}`，error 包含 code、中文 message、affectedIds 和 recoverableAction，与 T01 外层保持一致。
...
最终关系模型已在 [SQL 设计](./数据模型/project.sql)中给出；按[跨模块约定](./04-跨模块接口与数据约定.md)到各模块实施时拆分迁移，不在 T01 一次建立所有业务表。任务输入快照 T04 先独立存在，T05 再补完整成果依赖，避免互相等待。
