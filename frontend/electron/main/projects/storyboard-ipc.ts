...
import { storyboardRoutes } from "../../shared/storyboard";
import type { RuntimeSupervisor } from "../runtime/supervisor";
import { isTrustedSender } from "../security";
import { businessRequest } from "./client";

export function installStoryboardIpc(
  window: BrowserWindow,
  supervisor: RuntimeSupervisor,
  development: boolean,
  getCurrent: () => ProjectSession | null,
): () => void {
  for (const [name, route] of Object.entries(storyboardRoutes))
    ipcMain.handle(`storyboard:${name}`, async (event, ...args: unknown[]) => {
      try {
        if (
          window.isDestroyed() ||
          !isTrustedSender(
  ...
  return () => {
    for (const name of Object.keys(storyboardRoutes))
      ipcMain.removeHandler(`storyboard:${name}`);
  };
}
